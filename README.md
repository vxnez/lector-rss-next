...
### 3. Clasificación 100% IA (catálogo cerrado, reparto backend/frontend)
Catálogo cerrado de 25 categorías con descripciones mutuamente excluyentes, few-shot y JSON estricto `{categoria, confidence}`. El backend **siembra** al ingerir (`local` + confianza real; `manual` al reclasificar por PATCH) y el frontend **pule** por lotes: retoma `sin-ia` y `General`/nulo de cualquier método salvo `manual`; `local`/`gemini`/`groq` con categoría real no se reprocesan y ningún lado sobrescribe `manual`. Solo puede elegirse una categoría existente (validación insensible a acentos, default General). Umbral mínimo 80%: veredictos propios con menor confianza no se persisten y la noticia sigue pendiente; lo ya guardado por debajo se reprocesa. Respaldo por palabras clave (≥2 coincidencias, empate = sin veredicto) cuando la IA falla o responde tibio.
